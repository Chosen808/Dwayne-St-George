...
enum
{
    clown = - 1, abe, bob, col, dan, ed, fred, gav, hal, ian, jon, abi, bea,
...
{
    for (int i = abe; i <= jon && pref[this][i] != that; i++)
    {
        return i;
    }
}
...
int decide (int man1, int wife2)
{
    if (rank (man1, wife2) < rank (man1, pairs[man1]) && rank (wife2, man1) < rank (wife2, pairs[wife2]))
    {
        printf ("   %4s (w/ %4s) and %4s (w/ %4s) prefer each other" "over current pairing. \n", name[man1], name[pairs[man1]], name[wife2], name[pairs[wife2]]);
...
int option (int man1, int man2)
{
    // force checking all pairs; "||" would shortcircuit
    return decide (man1, pairs[man2] + decide (man2, pairs[man1]));
}
...
int unstable ()
{
    ...
    for (int i = abe; i < jon; i++)
    {
        for (int j = i + 1; j <= jon; j++)
        {
            if (option (i, j))
            {
                bad = 1;
...
main ()
{
    ...
    printf (unstable() ? "Relationship not stable \n" : "Stable matchup \n");

    printf ("\nBut if Bob Fred were to choose different: \n");
